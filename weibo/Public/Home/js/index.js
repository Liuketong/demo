		...
		$.ajax({
			...
			success : function (data, response, status) {
				if (data) {
					...
					switch (img.length) {
						...
						default :
							for (var i = img.length - 1; i >= 0; i--) {
								...
								$('#ajax_html3').find('p').after('<div class="imgs"><img src="' + ThinkPHP['ROOT'] + img_arr['thumb'].substr(1) + '" unfold-src="' + ThinkPHP['ROOT'] + img_arr['unfold'].substr(1) + '" source-src="' + ThinkPHP['ROOT'] + img_arr['source'].substr(1) + '"></div>');
							}
					...
					if (html.indexOf('#缩略图#')) {
						if (img['thumb']) {
							html = html.replace(/#缩略图#/g, ThinkPHP['ROOT'] + '/' + img['thumb'].substr(2));
						}
					}
					if (html.indexOf('#放大图#')) {
						if (img['unfold']) {
							html = html.replace(/#放大图#/g, ThinkPHP['ROOT'] + '/' + img['unfold'].substr(2));
						}
					}
					if (html.indexOf('#原图#')) {
						if (img['source']) {
							html = html.replace(/#原图#/g, ThinkPHP['ROOT'] + '/' + img['source'].substr(2));
						}
					}		
